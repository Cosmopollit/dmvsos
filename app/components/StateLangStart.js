...
// used by the home/header language switcher (no flags); picking a language +
// Start launches the practice test in that language.
//
// Pro-aware: a paying user is not "free", so the button drops the "Free" word
// (and the free-tier "20 questions" note hides) once auth resolves.
const LANGS = [
...
export default function StateLangStart({ state, lang = 'en', startFree = 'Start Free', startPro = 'Start Practicing', noteFree = '' }) {
  ...
  return (
    <>
      <div className="flex flex-col items-stretch gap-3">
        ...
        <GradientButton onClick={() => router.push(`/category?state=${state}&lang=${sel}`)} className="w-full">
          {isPro ? startPro : startFree}
        </GradientButton>
      </div>
