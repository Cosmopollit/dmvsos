...
export const EXPERIMENTS = {
  ...
  hero_copy: {
    status: 'running',
    description: 'Hero headline A/B: v1 control vs v3 biggest-bank (v2 uniqueness dropped)',
    variants: { v1: 0.5, v3: 0.5 },
...
// When cookies are blocked, the write below silently no-ops and every call
// would mint a fresh id, logging each pageview as a new "user" in exposure
// data. The in-memory fallback keeps the id stable for the page lifetime.
let anonIdFallback = null;
export function getAnonId() {
  ...
  if (anonIdFallback) return anonIdFallback;
  ...
  try {
    ...
    // Read back to verify the cookie stuck; if not, fall back to memory.
    if (!document.cookie.includes(`dmvsos_anon=${id}`)) anonIdFallback = id;
  } catch {
    anonIdFallback = id;
  }
  return id;
