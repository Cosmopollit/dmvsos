...
const LEGACY_PLAN_GRANTS = {
  car_pass:        { car: true },
  moto_pass:       { moto: true },
  cdl_pass:        { car: true, cdl: true },   // CDL Pro includes car tests
...
export function AuthProvider({ children }) {
  ...
  const fetchPasses = useCallback(async () => {
    try {
      ...
      // CDL Pro is sold as "Car tests included", so a CDL pass also unlocks car.
      let car = !!passes.auto || !!passes.cdl;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Webhook still hadn't landed after the full poll window (~62s). Mark
      // it so the next visibilitychange refetch bypasses the 30s throttle
      // once; the user's likely next move is tabbing away and back.
      try { sessionStorage.setItem('dmvsos_pass_recheck', '1'); } catch { /* private mode */ }
    })();
  ...
  useEffect(() => {
    ...
    function onVisible() {
      ...
      // One-shot throttle bypass set by the post-payment poller when the
      // webhook outlasted the whole poll window (cold starts).
      let bypass = false;
      try {
        bypass = sessionStorage.getItem('dmvsos_pass_recheck') === '1';
        if (bypass) sessionStorage.removeItem('dmvsos_pass_recheck');
      } catch { /* private mode */ }
      const now = Date.now();
      if (!bypass && now - lastVisibleFetchRef.current < 30000) return;
