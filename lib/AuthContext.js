...
// Plan types stored in profiles.plan_type, used by the legacy-profile
// fallback path below. Each one grants access to a set of categories.
//
// Two vocabularies coexist here on purpose:
//   - new per-type values (auto/moto/cdl) — what the Stripe webhook and the
//     manual-grant script write to profiles.plan_type today. These MUST be
//     present, otherwise a manually-granted or one-time user whose pass is
//     only reachable via the legacy fallback (e.g. the pass lives on a
//     different auth.users.id than the one they're signed in under) would
//     be silently treated as free.
//   - old subscription/one-time values (car_pass, quick_pass, …) — kept for
//     pre-migration accounts.
const LEGACY_PLAN_GRANTS = {
  // New per-type pass values (auto === car category).
  auto:            { car: true },
  moto:            { moto: true },
  cdl:             { cdl: true },
  // Legacy values.
