...
export function AuthProvider({ children }) {
  ...
  // Fetch passes whenever the signed-in identity changes, then poll a few
  // times if nothing was found yet. The poll closes the post-payment race:
  ...
  // /success 4s wait). Without this, AuthContext cached "free" and the user
  // saw the paywall right after paying until a manual hard refresh. Backoff
  // 2s/4s/8s covers ~14s of webhook lag, then stops.
  useEffect(() => {
  ...
  // open tab kept showing stale "free" until a full reload.
  useEffect(() => {
    if (!user?.id) return;
    function onVisible() {
      if (document.visibilityState === 'visible') fetchPasses();
    }
