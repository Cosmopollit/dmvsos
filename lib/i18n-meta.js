...
// ─────────────────────────────────────────────────────────────────────────
// Canonical consolidation (English-first).
//
// The SEO landing pages render an English body on EVERY ?lang= variant (only
// the <title>/<meta> were localized). Advertising 5 hreflang alternates that
// all serve duplicate English content made Google drop them as duplicates
// ("Crawled - currently not indexed") and distrust the hreflang. So we point
// the canonical of every variant at the single base URL and emit NO hreflang.
// Real per-lang hreflang returns once the bodies are genuinely localized via
// path-segment locale routes (/es/..., /ru/...).
// ─────────────────────────────────────────────────────────────────────────

const SITE_URL = 'https://dmvsos.com';

export function localizedAlternates(pathname) {
  return { canonical: `${SITE_URL}${pathname}` };
}
