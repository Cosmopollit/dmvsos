...
// ─────────────────────────────────────────────────────────────────────────
// Self-canonical & per-lang hreflang — replaces the EN-centric default.
// Returns the canonical URL pointing to THIS lang variant, plus all five
// hreflang alternates so each variant signals its own self + siblings.
// ─────────────────────────────────────────────────────────────────────────

const SITE_URL = 'https://dmvsos.com';

export function localizedAlternates(pathname, lang = 'en') {
  const base = `${SITE_URL}${pathname}`;
  const self = lang === 'en' ? base : `${base}?lang=${lang}`;
  const languages = {
    'x-default': base, // default → English variant
  };
  for (const [appCode, isoCode] of Object.entries(APP_LANG_TO_HTML_LANG)) {
    languages[isoCode] = appCode === 'en' ? base : `${base}?lang=${appCode}`;
  }
