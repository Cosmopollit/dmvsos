#!/usr/bin/env node
/**
 * Generate EN car questions for any state from its official driver manual.
 * Generic successor to generate-illinois-car.js (kept as the reference run).
 *
 * Generates ~300 questions across 20 topics, uploads directly to Supabase.
 * Run the rest of the pipeline afterward:
 *   node scripts/cluster-questions.js --state=<state>     (dedup, cap 200, codes)
 *   node scripts/translate-cluster.js --state=<state>     (RU/ES/ZH/UA rebuild)
 *   node scripts/verify-cluster-answers.js --state=<state>
 *   node scripts/count-state-questions.mjs                (refresh the manifest)
 *
 * Usage:
 *   node scripts/generate-state-car.js --state=maine [--dry-run]
 *
 * Requires: SUPABASE_SERVICE_ROLE_KEY, ANTHROPIC_API_KEY
 * Needs .manuals-text/<state>-car-en.txt (run extract-manuals.js first).
 */
...
if (!stateArg) { console.error('Usage: node scripts/generate-state-car.js --state=<slug>'); process.exit(1); }
if (['florida', 'washington'].includes(stateArg)) {
  // FL + WA car banks are finalized — no bulk regeneration allowed.
  console.error(`${stateArg} car is frozen; refusing to generate.`);
  process.exit(1);
}
...
const STATE_NAME = stateArg.split('-').map(w => w[0].toUpperCase() + w.slice(1)).join(' ');
...
const TOPICS = [
  ...
  `alcohol and drugs: ${STATE_NAME} DUI laws, BAC limits, consequences, implied consent`,
  `distracted driving: cell phones, texting, ${STATE_NAME} hands-free laws`,
  ...
  `${STATE_NAME} license requirements: classes, permits, graduated licensing, renewals`,
  `insurance and financial responsibility laws in ${STATE_NAME}`,
...
function findTopChunks(manualText, topic, topN = 4) {
  const stopWords = ['with', 'from', 'that', 'this', 'what', 'when', 'their', 'have', 'rules',
    ...STATE_NAME.toLowerCase().split(' ')];
...
async function generateTopic(topicNum, topic, manualText) {
  ...
  const prompt = `You are writing ${STATE_NAME} DMV knowledge test questions for new drivers preparing for their ${STATE_NAME} driver's license exam.

Topic: ${topic}

Relevant ${STATE_NAME} Driver Manual excerpts:
---
${context}
---

Generate exactly 15 multiple-choice questions about this topic. Requirements:
- Questions must reflect ${STATE_NAME}-specific laws and rules (not generic)
...
async function main() {
  ...
  const manualPath = path.join(__dirname, '..', '.manuals-text', `${stateArg}-car-en.txt`);
  if (!fs.existsSync(manualPath)) {
    console.error(`No manual text at ${manualPath} — run extract-manuals.js first.`);
    process.exit(1);
  }
  const manualText = fs.readFileSync(manualPath, 'utf8');
  console.log(`${STATE_NAME}: manual loaded, ${manualText.split(/\s+/).length} words\n`);

  const allQuestions = [];
  const backupFile = path.join(__dirname, '..', `.${stateArg}-car-questions.json`);
  ...
  console.log('\nNext steps:');
  console.log(`  node scripts/cluster-questions.js --state=${stateArg}`);
}
